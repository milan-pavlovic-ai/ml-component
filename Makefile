...
deploy: build
	...
	sam deploy --capabilities CAPABILITY_NAMED_IAM --profile $(PROFILE) --region $(REGION) --parameter-overrides "BucketParam=\"$(BUCKET)\" StageParam=\"$(PROFILE)\"" --stack-name "car-pricing" --config-file "samconfig.toml" --config-env $(PROFILE) --confirm-changeset
